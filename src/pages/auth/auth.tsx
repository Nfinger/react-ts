...
import { AuthError, AuthSubmission } from './types'


const AuthPage = () => {
    const [newAccount, toggleAuth] = useBoolean(false)
    const [location, setLocation] = useLocation();

    const [state, submitCreds] = useAsyncFn(async ({ email, password }) => {
        const authFunc = newAccount ? signUp : signIn
        await authFunc(email, password)
        setLocation('/dashboard')
    })

    const validate = (values: AuthSubmission) => {
        ...
        if (newAccount){
            ...
            if (values.password !== values.confirmPassword) {
                errors[FORM_ERROR] = "Passwords do not match"
            }
        }
        return errors
    }
    ...
    return (
        ...
        render={({ handleSubmit, submitError }) => (
            <form
                ...
                className="max-w-sm lg:min-w-md mx-auto my-6  overflow-hidden bg-white rounded-lg shadow-lg dark:bg-gray-800 lg:max-w-lg"
            >
                <div className="w-full px-6 py-8 md:px-8">
                    ...
                    <div className="mt-8">
                        ...
                        >{newAccount ? "Sign Up" : "Login"}</button>
                    </div>
                    ...
                    <div className="flex items-center justify-between mt-4">
                        ...
                        >{newAccount ? "or sign up" : "Already have an account?"}</span>
